...
if __name__ == '__main__':
    # 数据预处理，原始ratings和movies文件，转化成movie特征+用户评分矩阵
    ML_data_preproccessing.data_preprocess()

    # 导入数据
    movies_feature = pd.read_csv('../data/Movielens/ml-latest-small/movies_feature.csv', index_col=0) # 电影特征只在1.基于内容的算法中用到
    ...
    #
    # train.to_csv('../data/Movielens/ml-latest-small/pred_ratings_CB.csv')
    #
    # pred_CB = pd.read_csv('../data/Movielens/ml-latest-small/pred_ratings_CB.csv', index_col=0)
    #
    ...
    #
    #
    # # 2. 使用user-user的协同过滤算法来估计评分
    # print("Start User-User...")
    # count = 0
    # total = float(train.shape[0])
    # for idx, user in train.iterrows():
    #     unrated_index = user[user == 0].index.values
    #     unrated_index_ = list(map(int, unrated_index))
    ...
    #         print('Completed %d' % presentage + '%')
    #
    # train.to_csv('../data/Movielens/ml-latest-small/pred_ratings_CF.csv')
    #
    #
    # pred_CF = pd.read_csv('../data/Movielens/ml-latest-small/pred_ratings_CF.csv', index_col=0)
    #
    # nonzero_index = user_rating.values.nonzero()
    #
    # actual = user_rating.values[nonzero_index[0], nonzero_index[1]]
